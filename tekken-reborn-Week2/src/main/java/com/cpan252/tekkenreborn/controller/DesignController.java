...
public class DesignController {
    ...
    @GetMapping
    public String displayDesignPage(Model model) {
    ...
    @PostMapping
    public String processFighterAddition(@Valid @ModelAttribute Fighter fighter,
            @ModelAttribute("fighterPool") FighterPool pool, Errors errors, Model model) {
        if (errors.hasErrors()) {
            return "design";
        }
        model.addAttribute("fighter", this.fighter);
        pool.add(fighter);
